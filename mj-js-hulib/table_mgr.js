...
TableMgr.DumpTable = function() 
{
    for ( let i = 0; i < 9; i++ )
    {
        let name = "tbl/table_%d.tbl".replace( '%d', i );
        this.m_tbl[ i ].dump( name );
    }
    for ( let i = 0; i < 9; i++ )
    {
        let name = "tbl/eye_table_%d.tbl".replace( '%d', i );
...
TableMgr.DumpFengTable = function() 
{
    for ( let i = 0; i < 9; i++ )
    {
        let name = "tbl/feng_table_%d.tbl".replace( '%d', i );
        this.m_tbl[ i ].dump( name );
    }
    for ( let i = 0; i < 9; i++ )
    {
        let name = "tbl/feng_eye_table_%d.tbl".replace( '%d', i );
        this.m_tbl[ i ].dump( name );
    }
};